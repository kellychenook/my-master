...
module.exports = {
  ...
  chainWebpack: config => {
    config.resolve.alias
      .set('@', resolve('src')) // @符号代表src这个文件夹
      .set('_c', resolve('src/components')) // _c符号代表src/components这个文件夹
  },
  // 表示打包时不生成.map文件，可减少打包体积和打包速度
  productionSourceMap: false,

  devServer: {
    proxy: 'http://localhost:3000' // 需要代理的url,告诉开发服务器将任何未知请求代理到这个url
  }
}
